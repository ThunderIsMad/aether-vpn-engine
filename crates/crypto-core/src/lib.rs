...
mod tests {
    ...
    fn contract_noise_ik_two_messages_one_rtt() {
        ...
        // Замер раскладки (CI, прогон 35064845279): 3568 + 3424 B. Это в 2.9 раза больше
        // бюджета `02 §5` (1264 + 1136) — тоже Phase 0 finding Q12: в clatter msg1 несёт
        // `Skem` (статический KEM-ключ, 1184 B) и гибридный `E` (DH + KEM-эфемер), а msg2 —
        // и `Ekem` (ct), и статический KEM-ключ узла. Размеры пиннятся равенством, чтобы
        // изменение encoding ломало тест, а не уходило молча.
        let measured = (msg1_len, msg2_len);
        assert_eq!(
            measured,
            (3_568, 3_424),
            "замер раскладки handshake должен совпадать с зафиксированным (QUESTIONS Q12)"
        );
        assert!(
