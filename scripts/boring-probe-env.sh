...
#
# Статус: РАЗВЕДКА, не прод-зависимость. boring в workspace Cargo.toml НЕ добавлен,
# crates/ не тронут. Скрипт нужен, пока идёт оценка Reality-обложки; в прод-окружение
# его тащить не планируется — если boring когда-нибудь станет прод-зависимостью,
# недостающие звенья переехут в local-env.sh/CI, а этот скрипт удалят.
#
