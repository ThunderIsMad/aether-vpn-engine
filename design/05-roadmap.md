...
- [x] **Интеграционный тест ротации** (главный риск проекта): сессия с 3 потоками, ротация
      ...
      Отдельно — прогон с потерей на одном канале и прогон с потерей на обоих
      (выживание сессии: обе записи отброшены, continuity и окно дедупа не пострадали;
      буфер-фолбэк как механизм доставки записей, отправленных в мёртвый канал —
      вне Phase 0, см. Exit ниже и BLOCKER-1).
...
- [ ] Reality/VLESS — последний: либо Go-sidecar с xray-core, либо `boring` с контролем ClientHello.
  ...
  - [x] **Сертификат Accept-пути (Q23)** (2026-09-18): схема xtls/reality — per-node X25519
        ...
        проверено: юниты + live-loopback boring-клиент. Клиентская Aether-верификация
        сертификата (T1, 2026-09-19): rustls-клиент с кастомным kx-группом (клиент
        владеет keyshare), HMAC-верификация сертификата + Ed25519 CertificateVerify —
        live handshake с настоящей верификацией зелёный, негатив (чужой node_pub)
        проваливает handshake. Осталась инъекция Q22-тега в CH (гейт-аутентификатор).
  - [ ] **Reality live interop** — живой peer-тест: аутентифицированный клиент проходит на
        ...
        независимого DPI-инструмента. Игнор-тесты в крейте ждут живого пира (T1 закрыт;
        следующий шаг — инъекция Q22-тега в ClientHello).
