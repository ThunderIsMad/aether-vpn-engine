...
- Reality/TCP-байндинг: length-prefixed frames; HOL tradeoff задокументирован.
  ...
  ⇐ b132-2 (решение Q22 — peek-before-decrypt, живой сплайс вместо статического снимка):
  **двухслойное различение**. Слой 1 — **гейт по ОТКРЫТОМУ ClientHello до терминации TLS**
  (`gate_decision`): аутентификатор — 24 B HMAC (`crypto_core::probe_tag`, ключ `K_probe` —
  отдельный слой, `LABEL_PROBE`) над ClientHello с нулённым местом тега + окно слотов
  времени (анти-replay, Q21-класс диапазона); клиент кладёт тег в TLS-расширение
  `session_ticket` (RFC 5077 opaque-поле — равномерно-случайный вид, неотличим от шума и
  от билетов настоящего клиента сайта). Вердикты: `Accept` → TLS терминируется; `Relay` →
  **живой сплайс** (`relay_to_target`): TCP к сайту-мишени, сырые байты в обе стороны без
  интерпретации — пробник разговаривает с настоящим сайтом (настоящий сертификат/handshake),
  сервер ничего не расшифровывает — источник неотличимости (как в Reality/xtls-rprx-vision);
  `Reject` (байты вообще не ClientHello) → тихое закрытие — как перегруженный сайт.
  Fail-safe: тайм-аут коннекта, потолок байт (`RelaySpec`), аплинк недоступен → тихое
  закрытие без RST-сигнатуры. Слой 2 — первая запись внутри терминированного TLS на
  `K_cover` (`classify_first_record`); не прошла → тихое закрытие без ответа. Кадр:
