...
- Узел держит **in-memory consumed-ticket set** на эпоху (ключ `epoch_id ‖ sha256(ticket_blob)`,
  Задача 3.1/Q26: ключ не меняется, запись несёт `{attempts, last_transcript}` — счётчик
  **санкционированных** (PoP валиден) попыток `RESUME` и хеш транскрипта последней попытки
  (`sha256("aether-resume-v3" ‖ …)`; у ретрая всегда новый `client_nonce` ⇒ новый транскрипт).
  Ветка по consumed-записи: байт-в-байт повтор уже принятой попытки → `RESUME_NAK replay`
  (счётчик НЕ тратится, слот не двигается); новый транскрипт при остатке бюджета
  (`attempts < 2`) → PoP → `RESUME_ACK` (санкционированный ретрай `§3.7`); бюджет исчерпан →
  `RESUME_NAK budget` (запись не меняется — идемпотентный отказ). Инкремент — только после
  валидной `sig_client`: поддельные RESUME бюджет владельца не жгут (анти-DoS).
  **Ограничение Phase 0 (класс ограничения `gen_bases`/Q25):** бюджет per-node in-memory до
  персистентного consumed-set (Q20) — морф между узлами одного эпох-флота даст ≤ 2× номинала.
  Бюджет — мера против брутфорса PoP и «не мучить чужой consumed-set» (`§3.7`, Q18), не мера
  целостности: дедуп `(sid, seq)` и окна не зависят от него.
  **Инвариант «ровно один живой экземпляр сессии на sid»:** повторный Accept по тому же тикету
  (санкционированный ретрай) замещает единственное зеркало сессии узла — второй живой
  экземпляр `Session` с тем же `(sid, K_session)` не допускается (узловая точка RESUME-Accept
  держит `Option<Session>`, присваивание дропает прежний; клиент мутирует единственный
  экземпляр, не пересоздавая).
...
- Ретрай после потери ACK: новый `client_nonce` и новый `eph_client`, тот же ticket. Узел
  принимает ретрай при остатке бюджета consumed-записи (первая + не более одной повторной,
  `§3.7` Q18/Задача 3.1) и отвечает вторым `RESUME_ACK`; байт-повтор кадра — `replay` без
  траты бюджета.
...
`epoch = 0x03`, `expired = 0x04`, `budget = 0x05` (Задача 3.1/Q26: бюджет попыток RESUME
исчерпан, consumed-запись `§3.6`; без фолбэка на полный handshake — клиент откатывается
на старый канал и прекращает ретраи по этому тикету). Шифрования нет — NAK не несёт секрета, только причину.
...
| Бюджет попыток исчерпан (consumed-запись, Задача 3.1) | узел | `RESUME_NAK budget` (0x05), без фолбэка на полный handshake; клиент откатывается на старый канал |
