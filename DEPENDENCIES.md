...
| `rustls` (ring-provider) | T1-клиент Reality-верификации | Уже в дереве через quinn; для T1 задействованы кастомный `SupportedKxGroup` (клиент владеет keyshare), провайдер-level `SecureRandom` (захват ch_random) и `dangerous()` custom `ServerCertVerifier` (НЕ verify-none: HMAC + настоящая Ed25519 CertificateVerify). Проверено live-handshake против AcceptServer (boring) в одном процессе | — | — | 2026-09-19 | 180 дней |
