...
> **УСТАРЕЛО (2026-09-18):** этот отчёт описывает пробу ДО принятия boring в прод.
> С коммита `f226d05` boring — прод-зависимость workspace (`cover-reality`, пин 4.22
> в `[workspace.dependencies]`, сборка в обязательном rust-job). Отчёт сохранён как
> историческая разведка — история не переписывается.
...
На момент пробы это была разведка, не прод-зависимость: `boring` **не** был добавлен
в workspace `Cargo.toml`, в `crates/` не было изменений. Всё хозяйство пробы — вне репо, в
